...
| 文件 | 说明 |
| --- | --- |
| `sql_excel_converter.py` | 主程序(Tkinter GUI,彩色主题) |
| `requirements.txt` | 运行依赖 |
| `run.bat` | 直接用 Python 运行 |
| `build_exe.bat` | Nuitka 一键打包脚本(输出 `build\SQLExcelConverter.exe`) |
...
### ① SQL 输入:自由勾选 TableName + ColumnName

1. 输入格式选 **SQL 数据库**,填写连接信息后点 **连接并加载表**;
2. 点 **选择数据表** 下拉框,以**勾选形式**选择要导出的表;
3. 确认表后自动加载字段,点 **选择字段** 下拉框**多选勾选**要导出的字段(默认全选,支持全选/全不选);
4. 输出选 Excel / JSON / CSV / SQL(表拷贝,可选建表模式),开始转换。
...
成功后生成 `build\SQLExcelConverter.exe`(单文件,双击即用,无需安装 Python)。

> 若 onefile 打包失败(个别杀软误报或环境问题),可删除 `build_exe.bat` 中 `--onefile` 一行,
> 改为目录模式,产物在 `build\SQLExcelConverter.dist\SQLExcelConverter.exe`,整个文件夹一起分发。
...
SQLExcelConverter.exe --selftest
```
