...
设计说明:
    ...
    * JSON 采用“表头+行记录”结构(一级键=表名,HeaderFields=字段名列表,Data.RowN=第 N 行数据数组);
...
def build_json_document(top_key, columns, rows):
    ...
    header_fields = [str(col) for col in columns]
    data = {}
    for index, row in enumerate(rows, start=1):
        norm = normalize_row(row, len(columns))
        data[f"Row{index}"] = [to_json_value(value) for value in norm]
...
def write_json_multi_file(path, tables, mode="replace", log=None):
    ...
    每个表一个一级键(内含 HeaderFields/Data);mode=merge 时保留原文件
    其他一级键,覆盖同名键。
    """
    payload = {}
    total = 0
...
def load_json_stream(path, top_key):
    ...
    支持两种结构:
    * 行记录式(默认输出):{表名: {HeaderFields: [字段...], Data: {RowN: [值...]}}};
...
def run_self_test():
    ...
    # JSON 写读往返(覆盖行记录结构 + 同名键合并逻辑 + 旧版列数组兼容)
    try:
        ...
        with open(tmp, "r", encoding="utf-8") as fp:
            doc = json.load(fp)
        ok = (doc["info"]["HeaderFields"] == ["Id", "Name"]
              and doc["info"]["Data"]["Row1"] == [1, "张三"]
              and doc["info"]["Data"]["Row2"][-1] == "李四")
        header, gen = load_json_stream(str(tmp), "info")
        got = list(gen)
        ok = ok and header == ["Id", "Name"] and got[0] == [1, "张三"]
        ...
        with open(tmp, "w", encoding="utf-8") as fp:
            json.dump({"legacy": {"Id": [7, 8], "Name": ["甲", "乙"]}},
                      fp, ensure_ascii=False)
        header, gen = load_json_stream(str(tmp), "legacy")
        got = list(gen)
        ok = ok and header == ["Id", "Name"] and got[1] == [8, "乙"]
        tmp.unlink(missing_ok=True)
