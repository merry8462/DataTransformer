...
# ---------------------------------------------------------------------------
# 多表(分节) CSV:一个文件多个 [Sheet:表名] 分节
# ---------------------------------------------------------------------------
def read_csv_sections(path, delimiter, encoding):
    """解析分节 CSV,返回 {分节名: (表头, 数据行列表)};
...
def _write_csv_sections(path, delimiter, encoding, sections):
    """按 {分节名: (表头, 行列表)} 结构重建分节 CSV 文件。"""
    with open(path, "w", newline="", encoding=encoding) as fp:
        writer = csv.writer(fp, delimiter=delimiter, lineterminator="\n")
        first = True
        for name, (header, rows) in sections.items():
            if not first:
                writer.writerow([])
            first = False
            writer.writerow([f"[Sheet:{name}]"])
            if header:
                writer.writerow([to_csv_value(h) for h in header])
            for row in rows:
                writer.writerow(row)


def write_csv_multi_file(path, tables, delimiter=",", encoding="utf-8-sig",
                         mode="replace", log=None):
    """多表写出 CSV:tables 为可迭代的 (分节名, columns, row_iterator)。

    一个文件包含多个 [Sheet:表名] 分节(节内首行为表头);
    mode=merge 时保留原有分节,仅覆盖同名分节。
    """
    if mode == "merge" and os.path.exists(path):
        old = read_csv_sections(path, delimiter, encoding)
        sections = old if isinstance(old, dict) else {}
        total = 0
        for sheet, columns, rows in tables:
            rows_list = [
                [to_csv_value(v) for v in normalize_row(row, len(columns))]
            ...
            sections[sheet] = ([to_csv_value(h) for h in columns], rows_list)
            total += len(rows_list)
        _write_csv_sections(path, delimiter, encoding, sections)
        return total
...
class App(QMainWindow):
    ...
    def _build_vars(self):
        ...
        self.out_csv_multi = CSV_MULTI_LABELS[0]
    ...
    def _build_input_card(self, parent_lay):
        ...
        ex_lay = QHBoxLayout(self.file_excel_opts)
        ex_lay.setContentsMargins(0, 0, 0, 0)
        ex_lay.addWidget(QLabel("工作表:"))
        self.in_sheet_combo = QComboBox()
        self.in_sheet_combo.setMinimumWidth(320)
        ex_lay.addWidget(self.in_sheet_combo, 1)
        ex_lay.addStretch(1)
    ...
    def _build_output_card(self, parent_lay):
        ...
        out_csv_lay.addWidget(QLabel("多表CSV方式:"))
        self.out_csv_multi_combo = QComboBox()
        self.out_csv_multi_combo.addItems(CSV_MULTI_LABELS)
        self.out_csv_multi_combo.setMinimumWidth(280)
        out_csv_lay.addWidget(self.out_csv_multi_combo)
    ...
    def _run_multi_sql(self, conn, adapter, sources, out_key, output):
        ...
        if out_key == "Excel":
            ...
            if "paths" in output:
                ...
                # 分节单文件模式:一个文件多个 [Sheet:表名] 分节
                def tables():
                    for src, header, rows, cur in self._iter_sql_sources(conn, adapter, sources):
                        try:
                            yield (src["table"], header, rows)
                        finally:
                            self._close_cursor(conn, cur)
                total_all = write_csv_multi_file(
                    output["path"], tables(), delimiter=output["delimiter"],
                    encoding=output["encoding"], mode=output["mode"], log=self.log)
                self.log(f"✅ CSV 导出完成: {output['path']}"
                         f"({len(sources)} 个 [Sheet:表名] 分节,共 {total_all:,} 行)", "ok")
                self.bridge.info_s.emit(
                    f"导出完成,{len(sources)} 个分节,共 {total_all:,} 行:\n{output['path']}")
        elif out_key == "SQL":
    ...
    def on_convert(self):
        ...
        if in_key == "SQL":
            ...
            if not self.in_file_edit.text() or not os.path.isfile(self.in_file_edit.text()):
                ...
                return
            if not self.in_sheet_combo.currentText():
        ...
        elif out_key == "CSV" and multi_sql:
            delim = DELIM_LABELS[self.out_delim_combo.currentText()]
            enc = ENC_LABELS[self.out_enc_combo.currentText()]
            multi_mode = self.out_csv_multi_combo.currentText()
            if multi_mode == CSV_MULTI_LABELS[1]:
                # 每表一个文件:选择输出目录
                out_dir = QFileDialog.getExistingDirectory(
                    self, "选择 CSV 输出目录(每张表生成一个 .csv 文件)")
                if not out_dir:
                    return
                paths = [os.path.join(out_dir, f"{safe_filename(t['table'])}.csv")
                         for t in sources]
                ...
                if path is None:
                    return
                output = {"path": path, "mode": conflict_mode,
                          "delimiter": delim, "encoding": enc}
        else:
            default_base = self._default_output_base(in_key)
            path, conflict_mode = self._ask_output_path(out_key, default_base)
...
def run_self_test():
    ...
    # 分节(多表) CSV 写读往返:一个文件多个 [Sheet:表名] 分节
    try:
        tmp = log_path.parent / "_selftest_multi.csv"
        tables = [
            ("NC7Middleschool", ["id", "name"], iter([[1, "张三"], [2, "李四"]])),
            ("departments", ["did", "dname"], iter([[10, "研发部"]])),
        ]
        write_csv_multi_file(str(tmp), tables)
        sections = read_csv_sections(str(tmp), ",", "utf-8-sig")
        ok = (isinstance(sections, dict)
              and set(sections) == {"NC7Middleschool", "departments"}
              and sections["NC7Middleschool"][1][0] == ["1", "张三"])
        header, gen = load_csv_section(str(tmp), "departments", ",", "utf-8-sig")
        rows = list(gen)
        ok = ok and header == ["did", "dname"] and rows[0] == ["10", "研发部"]
        tmp.unlink(missing_ok=True)
        lines.append(f"[{'OK' if ok else 'FAIL'}]   分节CSV(多Sheet)写读往返")
    except Exception as exc:
        lines.append(f"[FAIL] 分节CSV 写读往返: {exc}")
