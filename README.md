...
| 层级 | 对应关系 | 示例 |
| --- | --- | --- |
| 数据库名 | == Excel/JSON 文件名 == **CSV 目录名** | `db0829.xlsx` / `db0829.json` / `db0829/` |
| 表名 | == Excel Sheet 名 == JSON 一级参数 == **CSV 目录下的 .csv 文件名** | `"info"` → `db0829/info.csv` |
| 字段名 | == Excel 首行单元格 == JSON 二级参数 == CSV 表头单元格 | `"Id"`、`"Name"`、`"CreateTime"` … |

即:导出 CSV 时先创建 **目录(目录名 = 数据库名)**,再按数据库表个数在目录下生成 **每个表一个 `表名.csv` 文件**。
...
3. 输出与多表的对应关系:
   ...
   - **CSV**:创建 **目录(目录名 = 数据库名)**,目录下每个表一个 **`表名.csv` 文件**;
   - **SQL**:多表同名表拷贝,目标表名请留空(按源表同名建表);
4. 开始转换。

> CSV 说明:标准 CSV 格式不支持多 Sheet,因此 SQL→CSV 采用"目录 + 每表一个文件"的方式,
> 与 Excel 多 Sheet / JSON 多一级键一一对应;目录内文件可直接用 Excel 打开。
> 旧版生成的 `[Sheet:表名]` 分节 CSV 仍可作为**输入**读取(程序自动识别并列出分节供选择)。
...
- **CSV**:支持分隔符(逗号 / 分号 / TAB / 竖线)与编码(UTF-8 BOM / UTF-8 / GBK)选择;
  选择文件后自动识别 **普通 CSV** 或 **旧版分节 CSV**,分节 CSV 可下拉选择要转换的 `[Sheet:表名]` 分节。
...
| **合并写入** | 保留其他 Sheet,覆盖同名 Sheet | 保留其他一级键,覆盖同名键 | 追加数据行 |
| **取消** | 不写入 | 不写入 | 不写入 |

> SQL→CSV(目录模式)会一次性检测目录内所有同名 .csv 文件,再统一询问覆盖/追加/取消。
