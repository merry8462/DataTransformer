...
class CheckboxDropdown(ttk.Frame):
    ...
    def set_hint(self, text):
        """清空选项并显示提示文本(用于多表导出时禁用字段选择)。"""
        self._items = []
        self._checked = set()
        self.button.configure(text=str(text), state="disabled")
...
class App:
    ...
    def _build_ui(self):
        ...
        log_btn_box = ttk.Frame(frm_log, style="Card.TFrame")
        log_btn_box.grid(row=0, column=1, sticky="ne", padx=6, pady=6)
        ttk.Button(log_btn_box, text="一键清空", style="Blue.TButton",
                   command=self.on_clear_log).pack()
    ...
    def _build_input_card(self, parent):
        ...
        self.table_dropdown = CheckboxDropdown(
            self.src_sql_frame, text="选择数据表(可多选勾选)", multi=True, width=40)
        ...
        ttk.Label(self.src_sql_frame,
                  text="可多选数据表;多表导出时每张表对应一个 Sheet / JSON 键(字段全选)",
    ...
    def _load_config(self):
        ...
        if isinstance(data, dict):
            if data.get("password"):
                self.pwd_var.set(str(data["password"]))
            if data.get("database"):
                self.database_var.set(str(data["database"]))
    ...
    def _save_config(self):
        ...
        try:
            ...
            path.write_text(
                json.dumps({"password": self.pwd_var.get(),
    ...
    def on_tables_confirmed(self, selected):
        self.selected_tables = [s for s in selected]
        if not self.selected_tables:
            self.selected_table = None
            self.column_dropdown.set_items([])
            return
        self.selected_table = self.selected_tables[0]
        self._auto_fill_output_names()

        if len(self.selected_tables) > 1:
            # 多表导出:每张表全部字段,字段下拉框禁用并给出提示
            self.column_dropdown.set_hint("多表导出:将导出每张表的全部字段")
            self.log(f"已勾选 {len(self.selected_tables)} 张表: "
                     f"{', '.join(self.selected_tables)}(导出全部字段)", "ok")
            return

        table = self.selected_tables[0]
    ...
    def _iter_sql_sources(self, conn, adapter, sources):
        """逐表流式读取,产出 (source, header, rows, cur);调用方负责关闭 cur。"""
        for src in sources:
    ...
    def _run_multi_sql(self, conn, adapter, sources, out_key, output):
        """多表 SQL 输入:Excel→多 Sheet,JSON→多一级键,CSV→多文件,SQL→同名表拷贝。"""
        total_all = 0
        if out_key == "Excel":
            def tables():
            ...
            self.msg_queue.put(
                ("info", f"导出完成,{len(sources)} 个 Sheet,"
                         f"共 {total_all:,} 行:\n{output['path']}"))
        elif out_key == "JSON":
            def tables():
                for src, header, rows, cur in self._iter_sql_sources(conn, adapter, sources):
            ...
            for idx, (src, header, rows, cur) in enumerate(
                ...
                try:
                    ...
                    self._close_cursor(conn, cur)
            self.msg_queue.put(
                ("info", f"导出完成,共 {total_all:,} 行,"
                         f"{len(sources)} 个 CSV 文件"))
        elif out_key == "SQL":
            ...
            try:
                for src, header, rows, cur in self._iter_sql_sources(
                    ...
                    def chained():
                        for r in buffered:
                            yield r
                        for r in rows:  # rows 生成器保留剩余未读数据
                            yield list(r)

                    target = src["table"]  # 多表同名拷贝
            ...
            self.msg_queue.put(
                ("info", f"写入完成,共 {total_all:,} 行 → {len(sources)} 张表"))
        return total_all
    ...
    def on_convert(self):
        ...
        sources = []
        if in_key == "SQL":
            tables = [t for t in self.selected_tables]
            if not tables:
                messagebox.showwarning("提示", "请连接数据库,并在“选择数据表”下拉框中勾选至少一张数据表")
                return
            if len(tables) == 1:
                ...
                if not columns:
                    ...
                    return
                sources = [{"table": tables[0], "columns": columns}]
            else:
                # 多表导出:每张表全部字段
                sources = [{"table": t, "columns": None} for t in tables]
        elif in_key == "Excel":
        ...
        if out_key == "SQL":
            ...
            if multi_sql:
                if table_out:
                    messagebox.showwarning("提示", "多表拷贝时目标表名请留空(将按源表同名建表)")
                    return
            elif not table_out:
            ...
            if mode == "replace":
                ...
                if not messagebox.askyesno("危险操作", msg):
                    return
            output = {"table": table_out or None, "mode": mode, "batch": batch}
        elif out_key == "CSV" and multi_sql:
            # 多表导出 CSV:选择一个目录,每张表一个 .csv 文件
            out_dir = filedialog.askdirectory(
                title="选择 CSV 输出目录(每张表生成一个 .csv 文件)")
            if not out_dir:
                return
    ...
    # ------------------------------------------------------------- 退出
    def on_close(self):
        self._save_config()  # 退出前记住密码与数据库名
        self._disconnect_conn()
        self.root.destroy()
